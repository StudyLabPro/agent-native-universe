...
| L5b single-machine live stack — `compose.live.yml`, the host-sized universe config, tiers, bootstrap, systemd units and `deploy/mws/DEPLOY_LIVE.md` | authored and locally exercised: the image builds, epoch 0 of the sized universe ran end to end against a real gateway and a stub provider (250 ticks, 23 545 events, 4 000 consultations, ~18.2 MB), and the runbook's disk thresholds come from that measurement. NOT deployed — blocked on the owner attaching `anu-live-1-ip`, creating the three secret versions and assigning the IAM role bindings. Five deviations from the multi-host design are recorded in `DEPLOY_LIVE.md` §2, the provider key sharing a host with the evidence first among them |
