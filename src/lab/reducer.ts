...
import {
  ...
  isExternalTask,
  learningFamilyOf,
  removeArchivedMessage,
  removeArchivedSubmission,
  removeArchivedTask,
} from "./epoch-rules.js";
...
export function prepareWorldEventTransition(
  ...
  switch (event.type) {
    ...
    /* ---------------------------------------------------------------- */
    /* The bounded world (phase L3c). Archiving removes a settled record  */
    /* from the live state; it never removes it from the evidence, which  */
    /* is the append-only log this state is reduced from. The reducer     */
    /* enforces only what makes the removal structurally safe — the       */
    /* record exists, it has settled, and nothing left in the state still */
    /* points at it. WHICH records an upkeep must archive is the window   */
    /* rule of `epoch-rules.ts`, regenerated by the protocol verifier.    */
    /* ---------------------------------------------------------------- */
    case "task.archived": {
      if (state.mode !== "live") throw new Error("Archival is a live-only rule");
      assertPhase(event, "upkeep");
      assertSystemEvent(event, "task.archived");
      const taskId = requiredString(data.taskId, "taskId");
      const task = requireTask(state, taskId);
      assertExactKeys(data, ["taskId"], "task.archived data");
      if (task.status !== "completed" && task.status !== "expired") {
        throw new Error(`Task ${taskId} in status ${task.status} cannot be archived`);
      }
      for (const submission of Object.values(state.submissions)) {
        if (submission.taskId === taskId) {
          throw new Error(`Task ${taskId} cannot be archived while submission ${submission.id} names it`);
        }
      }
      mutation = () => {
    ...
    case "submission.archived": {
      if (state.mode !== "live") throw new Error("Archival is a live-only rule");
      assertPhase(event, "upkeep");
      assertSystemEvent(event, "submission.archived");
      const submissionId = requiredString(data.submissionId, "submissionId");
      const submission = requireSubmission(state, submissionId);
      assertExactKeys(data, ["submissionId"], "submission.archived data");
      const task = state.tasks[submission.taskId];
      if (task !== undefined && task.status !== "completed" && task.status !== "expired") {
        throw new Error(`Submission ${submissionId} cannot be archived while its task is ${task.status}`);
      }
      mutation = () => {
        removeArchivedSubmission(state, submissionId);
      };
      break;
    }
    case "message.archived": {
      if (state.mode !== "live") throw new Error("Archival is a live-only rule");
      assertPhase(event, "upkeep");
      assertSystemEvent(event, "message.archived");
      const messageId = requiredString(data.messageId, "messageId");
      const message = requireMessage(state, messageId);
      assertExactKeys(data, ["messageId"], "message.archived data");
      if (message.deliveredTick === undefined) {
        throw new Error(`Message ${messageId} cannot be archived before it is delivered`);
      }
      mutation = () => {
        removeArchivedMessage(state, messageId);
      };
      break;
    }
